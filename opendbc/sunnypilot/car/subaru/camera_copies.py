...
# it about a quarter second later (2026-09-22): the driver's own press moves Throttle_Combo and the
# engine with the pedal, so the copy carries all three, and the tap holds until the car is moving
...
class CameraCopiesController:
  ...
  def create_camera_copies(self, packer, CC: structs.CarControl, CS: CarStateBase, frame: int) -> list[CanData]:
    ...
    can_sends.append(subarucan_ext.create_throttle_echo(packer, frame, CS.throttle_msg, throttle_cruise,
